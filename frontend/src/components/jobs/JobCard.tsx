...
import { categoryIcon, categoryColors } from "@/lib/utils";
...
function CategoryAvatar({ category, size }: { category: string; size: "sm" | "lg" }) {
  const initials = categoryIcon(category);
  const [bg, fg] = categoryColors(category);
  const dim = size === "lg" ? "w-12 h-12 text-sm" : "w-11 h-11 text-xs";
  return (
    <div
      className={`${dim} rounded-xl flex items-center justify-center font-bold flex-shrink-0 select-none`}
...
export default function JobCard({ job, view = "grid" }: Props) {
  ...
  if (view === "list") {
    return (
      ...
      >
        <CategoryAvatar category={job.category} size="sm" />
        ...
        <div className="flex-shrink-0 text-right min-w-[80px]">
          ...
          <div
            className="flex items-center gap-1 text-xs justify-end mt-0.5"
            style={{ color: "var(--outline)" }}
          >
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <CategoryAvatar category={job.category} size="lg" />
        <BookmarkButton jobId={job.id} jobTitle={job.title} />
      </div>
