...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 w-full z-50 transition-shadow duration-200"
      style={{
      ...
      <nav className="flex justify-between items-center w-full px-4 md:px-10 max-w-[1280px] mx-auto h-16"
        aria-label="Main navigation">

        {/* Logo */}
        <div className="flex items-center gap-6">
          <Link href="/" aria-label="Tawtheef Egypt home" className="flex items-center gap-2 select-none">
            <Image src="/logo.png" alt="Tawtheef Egypt logo" width={32} height={32}
            ...
            <span className="font-bold text-base hidden sm:block"
              ...
              Tawtheef <span style={{ color: "var(--primary-container)" }}>Egypt</span>
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1" role="navigation">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}
                ...
                style={{
                  color: isActive(link.href) ? "var(--primary-container)" : "var(--on-surface-variant)",
          ...
          <Link href="/saved"
            ...
            {bookmarks.size > 0 && (
              <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[10px] font-bold flex items-center justify-center text-white"
                style={{ background: "var(--primary-container)" }} aria-hidden="true">
                {bookmarks.size}
              </span>
            )}
          </Link>
      ...
      <div id="mobile-menu"
        ...
        style={{ background: "rgba(255,255,255,0.98)", borderColor: "var(--outline-variant)" }}
        aria-hidden={!mobileOpen}>
        {navLinks.map((link) => (
          <Link key={link.href} href={link.href}
            className="px-3 py-2 rounded-lg text-sm font-medium"
            style={{
              color: isActive(link.href) ? "var(--primary-container)" : "var(--on-surface)",
              background: isActive(link.href) ? "rgba(0,33,71,0.06)" : "transparent",
              fontWeight: isActive(link.href) ? 600 : 500,
            }}
            aria-current={isActive(link.href) ? "page" : undefined}>
        ...
        <Link href="/post-job" className="btn-primary px-4 py-2 text-sm text-center">
          Post a Job
        </Link>
      </div>
    </header>
